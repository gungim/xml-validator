...
export async function GET(
  ...
  try {
    ...
    return createErrorResponse('Failed to fetch rule', 500)
  }
}
  ...
  try {
    ...
    if (body.dataType && body.dataType !== existingRule.dataType) {
      ...
      if (existingRule.children && existingRule.children.length > 0) {
        return createErrorResponse(
          'Cannot change data type of a rule with children',
          400
        )
      }
    ...
    if (body.globalRuleId !== undefined && body.globalRuleId !== null) {
      ...
      if (globalRule.dataType !== targetDataType) {
        return createErrorResponse(
          `Data type mismatch. Global rule is ${globalRule.dataType}, but rule is ${targetDataType}`,
          400
        )
      }
    }
    ...
    if (body.globalRuleId !== undefined)
      updateData.globalRuleId = body.globalRuleId

    // If path is changing and rule has children (object/array), update children paths
    if (
      body.path &&
      body.path !== existingRule.path &&
      existingRule.children &&
      existingRule.children.length > 0
    ) {
      ...
      async function updateChildrenPaths(
        ...
        for (const child of children) {
          ...
          if (child.path.startsWith(oldParentPath + '.')) {
            newChildPath = child.path.replace(
              oldParentPath + '.',
              newParentPath + '.'
            )
          }
